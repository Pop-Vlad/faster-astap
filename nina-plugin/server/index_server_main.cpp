...
namespace {
  ...
  private:
    Message solve(const Message &req) {
      ...
      r.params.fov = req.number("fov", 0);
      r.params.max_stars = req.integer("maxstars", 500);
      r.params.min_star_size = req.number("minstar", 1.5);
      r.params.downsample = req.integer("downsample", 0);
      r.write_wcs = req.flag("wcs", false);
      r.params.want_sip = req.flag("sip", false);
      r.params.refine = req.flag("refine", true);
...
int main(int argc, char **argv) {
  ...
  for (const std::string &filename: images) {
    ...
    r.params.fov = has("fov") ? std::atof(val("fov").c_str()) : 0;
    r.params.max_stars = has("s") ? std::atoi(val("s").c_str()) : 500;
    r.params.min_star_size = has("m") ? std::atof(val("m").c_str()) : 1.5;
    r.params.downsample = has("z") ? std::atoi(val("z").c_str()) : 0;
    r.write_wcs = has("wcs");
    r.params.want_sip = has("sip");
    r.params.refine = !has("norefine");
    r.cmdline = cmdline;
    astap::LogFn report;
